...
namespace Web_Api.online.Data.Repositories
{
    public class WalletsRepository
    {
        ...
        public async Task<WalletTableModel> GetUserWalletAsync(string userId, string acronim)
        {
            try
            {
                WalletTableModel result = await _db.QueryFirstOrDefaultAsync<WalletTableModel>("spGetUserWalletByAcronim",
                new
                {
                    userId = userId,
                    acronim = acronim
        ...
        public async Task<IncomeWalletTableModel> CreateUserIncomeWalletAsync(IncomeWalletTableModel wallet)
        {
            try
            {
                ...
                await _db.QueryAsync<int>("spCreateUserIncomeWallet", p, commandType: CommandType.StoredProcedure);
        ...
        public async Task<WalletTableModel> CreateUserWalletAsync(WalletTableModel wallet)
        {
            try
            {
                ...
                await _db.QueryAsync<int>("spCreateUserWallet", p, commandType: CommandType.StoredProcedure);
        ...
        public async Task UpdateWalletBalance(WalletTableModel wallet)
        {
            try
            {
                await _db.QueryAsync<WalletTableModel>("spUpdateWalletBalance",
                    new
                    {
                        walletId = wallet.Id,
        ...
        public async Task<List<spGetNotEmptyValueWallet_ByUserId>> GetNotEmptyWalletsByUserId(string userId)
        {
            try
            {
                return (await _db.QueryAsync<spGetNotEmptyValueWallet_ByUserId>("spGetNotEmptyValueWallet_ByUserId",
                    new
                    {
                        userid = userId
                    },
        ...
        public async Task SendCoinsAync(SendCoinsModel sendCoinsModel)
        {
            try
            {
                await _db.QueryAsync<WalletTableModel>("spSendCoins",
                    new
                    {
